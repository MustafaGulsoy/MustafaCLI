...
class ReportGenerator:
    ...
    async def generate(
        ...
        if fmt == ReportFormat.CLI:
            ...
            return await self._render_neo4j(results, rid)
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <p style="color:#94a3b8;margin-bottom:1rem">Open <a href="http://localhost:7474/browser/" style="color:#3b82f6">Neo4j Browser</a> and run these queries:</p>
            <table>
                <thead><tr><th>Query</th><th>Cypher</th></tr></thead>
                <tbody>
                    <tr><td>All Components</td><td><code>MATCH (c:Component)-[:HAS_PIN]-&gt;(p:Pin) RETURN c, p</code></td></tr>
                    <tr><td>Violations</td><td><code>MATCH (v:Violation) RETURN v ORDER BY v.severity</code></td></tr>
                    <tr><td>Power Network</td><td><code>MATCH (p:Pin {{direction: 'POWER'}})-[:CONNECTED_TO*]-(p2) RETURN p, p2</code></td></tr>
                    <tr><td>Connector Pairs</td><td><code>MATCH (c1:Connector)-[:MATES_WITH]-&gt;(c2) RETURN c1, c2</code></td></tr>
                    <tr><td>PCB Data</td><td><code>MATCH (p:Pad) RETURN p LIMIT 100</code></td></tr>
                </tbody>
            </table>
        </div>
    ...
    async def _render_neo4j(self, results: list[AnalysisResult], run_id: str) -> str:
        """Store results in Neo4j and return Browser deep links."""
        lines = [
            f"Results stored in Neo4j (run: {run_id})",
            "",
            "Neo4j Browser Links:",
            f"  Dashboard: http://localhost:7474/browser/",
            "",
            "Useful Cypher Queries:",
            f"  All violations:     MATCH (run:AnalysisRun {{id: '{run_id}'}})-[:FOUND]->(v:Violation) RETURN v",
            f"  Component tree:     MATCH (c:Component)-[:HAS_PIN]->(p:Pin) RETURN c, p",
            f"  Power connections:  MATCH (p:Pin {{direction: 'POWER'}})-[:CONNECTED_TO*]-(p2:Pin) RETURN p, p2",
            f"  Connector pairs:    MATCH (c1:Connector)-[:MATES_WITH]->(c2:Connector) RETURN c1, c2",
            f"  ECSS rules:         MATCH (r:EcssRule) RETURN r ORDER BY r.severity",
            f"  Error violations:   MATCH (v:Violation {{severity: 'ERROR'}}) RETURN v",
            f"  PCB pads:           MATCH (p:Pad) RETURN p LIMIT 100",
            f"  PCB traces:         MATCH (t:Trace) RETURN t LIMIT 100",
        ]

        # Add per-analyzer deep links
        for r in results:
            if r.violations:
                cypher = f"MATCH (run:AnalysisRun)-[:FOUND]->(v:Violation) WHERE run.analyzer = '{r.analyzer}' RETURN v"
                lines.append(f"  {r.analyzer} violations: {cypher}")
